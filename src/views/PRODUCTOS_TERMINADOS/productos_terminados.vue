<template>
  <v-main class="pa-0 ma-3">
    <v-row justify="center">
          ...
          <v-data-table
           ...
           >
            ...
            <template v-slot:item.fecha_actualiza="{ item }">
              <span class="black--text"> {{ moment(item.fecha_actualiza).format('LL') }} </span>
            </template>
            ...
            <template v-slot:item.action="{ item }">
              <v-btn 
                color="success" 
                dark  small 
                @click="salida_material_modal=true; parametros=item;"
              >
      ...
      <v-dialog v-model="salida_material_modal" width="600px" persistent transition="dialog-bottom-transition">
        <v-card class="pa-3">
          <SMATERIAL
            :parametros="parametros"
            @modal="salida_material_modal = $event"
          />
        </v-card>
      </v-dialog>
...
<script>
  ...
  import SMATERIAL from '@/views/SALIDAS/salida_material.vue'
  ...
  export default {
    ...
    data:() =>({
      ...
      headers: [
          ...
          { text: 'Cantidad existente'   , align: 'left'  , value: 'cantidad' },
          { text: 'Unidad'     , align: 'left'  , value: 'unidad'   },
          { text: 'Sucursal'   , align: 'left'  , value: 'nomsucursal' },
          { text: 'Ultima Actualización'   , align: 'left'  , value: 'fecha_actualiza' },
					{ text: '' 		      , align: 'right' , value: 'action' , sortable: false },
      ],

      salida_material_modal: false,
      parametros: {}
    }),
    ...
    methods:{ 
      ...
      async init(){
        const payload = { id_sucursal: this.sucursal.id };
