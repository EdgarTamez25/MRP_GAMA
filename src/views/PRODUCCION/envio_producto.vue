<template>
  <v-row justify="center">
    ...
    <v-col cols="12" sm="6" class=" text-center  ">
      <v-select
        v-model="datos.tipo_envio" :items="tipos_envios" item-text="nombre" item-value="id"  
          hide-details  placeholder="TIPO DE ENVÍOS " return-object outlined label="TIPO DE ENVÍOS"
      ></v-select> 
    </v-col>
    ...
    <v-col cols="12" sm="6" class="">
        <v-select
        v-model="datos.deptos" :items="departamentos" item-text="nombre" item-value="id" color="celeste" 
        outlined  hide-details  placeholder="DEPARTAMENTO A ENVÍAR" return-object label="DEPARTAMENTO A ENVÍAR"
      ></v-select>
    </v-col>
    ...
    <v-dialog v-model="alerta_envio_material" persistent max-width="500">
      <v-card >
        <v-card-title class="subtitle-1 font-weight-black" style="word-break:normal;" > SE AUTORIZARÁ EL ENVÍO DEL PRODUCTO CON LA CANTIDAD DE {{ datos.cantidad_enviar | currency(0) }}  </v-card-title>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      VALIDA_DATOS(){
          return this.datos.tipo_envio.id != null && 
                 this.datos.deptos.id != null && 
